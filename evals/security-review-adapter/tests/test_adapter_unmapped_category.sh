...
(cd "$REPO_ROOT" && python3 "$ADAPTER" --input "$FXT" --output "$OUT" \
  --mapping plugins/dev-team/knowledge/security-review-rule-map.yaml 2>"$ERR")
rc=$?
set -e
if [[ $rc -ne 0 ]]; then
...
expected="WARN: category A99.new-class not in mapping at plugins/dev-team/knowledge/security-review-rule-map.yaml; minted security-review.a99.new-class"
