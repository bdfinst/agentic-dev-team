...
- CLI: `shellcheck`, `jq`, `python3`, `uv` (macOS: `brew install shellcheck jq python3 uv`; `uv` otherwise via `curl -LsSf https://astral.sh/uv/install.sh | sh`). `shellcheck` still lints repo-root shell (`scripts/audit-rules-vs-prompts.sh`, etc.) and the `plugins/security-assessment/` plugin; the `plugins/dev-team/` plugin itself is now Python. `uv` provisions the Python 3.10 floor interpreter and runs the floor-interpreter test slice (`chk_python_floor` in `scripts/ci-local.sh`) — the default pre-push gate fails outright without it, not just the `Python 3.10 floor` CI job.
