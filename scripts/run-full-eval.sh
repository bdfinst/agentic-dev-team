...
if [ -z "$ONLY_AGENT" ]; then
  ...
  if [ -n "$(git log origin/main..HEAD --oneline 2>/dev/null)" ]; then
    # Push VISIBLY: the pre-push hook runs scripts/ci-local.sh and blocks on
    # failure — suppressing it hides the real reason. Work is safe on $BRANCH.
    echo "== pushing $BRANCH (pre-push runs the local CI mirror)… =="
    git push -u origin "$BRANCH" || {
      echo "push failed — see the pre-push / CI output above. Work is committed on" >&2
      echo "'$BRANCH'; re-push with: git push -u origin $BRANCH  (or HUSKY=0 to bypass)." >&2
      exit 2
    }
