...
HOOK="$BATS_TEST_DIRNAME/../../plugins/agentic-dev-team/hooks/pre-commit-knowledge-index.sh"
DETECT_LIB="$BATS_TEST_DIRNAME/../../plugins/agentic-dev-team/hooks/lib/pre-commit-detect.sh"
...
setup() {
  ...
  mkdir -p plugins/agentic-dev-team/{knowledge,skills/specs,agents,hooks/lib}
  cat > plugins/agentic-dev-team/knowledge/foo.md <<'EOF'
# Foo
## Section A
First sentence of A.
EOF
  cat > plugins/agentic-dev-team/agents/some-agent.md <<'EOF'
# Some Agent
## Section
Body.
EOF
  ...
  mkdir -p plugins/agentic-dev-team/hooks/lib
  cp "$BATS_TEST_DIRNAME/../../plugins/agentic-dev-team/hooks/lib/knowledge-index-paths.sh" plugins/agentic-dev-team/hooks/lib/
  ...
  bash plugins/agentic-dev-team/hooks/lib/build-knowledge-index.sh
  git add plugins/agentic-dev-team/knowledge/index.json
}
...
@test "AC15: non-commit bash invocations are silent no-ops" {
  ...
  run bash -c "cd '$BATS_TMPDIR_CASE' && echo '$input' | bash plugins/agentic-dev-team/hooks/pre-commit-knowledge-index.sh 2>&1"
  [ "$status" -eq 0 ]
  [ -z "$output" ]
}
...
@test "AC15: commit with only non-corpus files staged is allowed" {
  ...
  cd "$BATS_TMPDIR_CASE"
  echo "## New" >> plugins/agentic-dev-team/agents/some-agent.md
  echo "Body." >> plugins/agentic-dev-team/agents/some-agent.md
  git add plugins/agentic-dev-team/agents/some-agent.md
...
@test "AC14: commit with corpus + matching index staged exits 0" {
  ...
  echo "## New Section" >> plugins/agentic-dev-team/knowledge/foo.md
  echo "Body of new section." >> plugins/agentic-dev-team/knowledge/foo.md
  bash plugins/agentic-dev-team/hooks/lib/build-knowledge-index.sh
  git add plugins/agentic-dev-team/knowledge/foo.md plugins/agentic-dev-team/knowledge/index.json
...
@test "AC13: commit with stale index blocks with the two-line remediation" {
  ...
  echo "## New Section" >> plugins/agentic-dev-team/knowledge/foo.md
  echo "Body of new section." >> plugins/agentic-dev-team/knowledge/foo.md
  git add plugins/agentic-dev-team/knowledge/foo.md
  ...
  [[ "$output" == *"bash plugins/agentic-dev-team/hooks/lib/build-knowledge-index.sh"* ]]
  [[ "$output" == *"git add plugins/agentic-dev-team/knowledge/index.json"* ]]
}
...
@test "AC13a: commit blocks when working tree drifts past a consistent staged pair" {
  ...
  echo "## First" >> plugins/agentic-dev-team/knowledge/foo.md
  echo "Body of first." >> plugins/agentic-dev-team/knowledge/foo.md
  bash plugins/agentic-dev-team/hooks/lib/build-knowledge-index.sh
  git add plugins/agentic-dev-team/knowledge/foo.md plugins/agentic-dev-team/knowledge/index.json
  # 2. Re-edit the working tree without re-staging.
  echo "## Second" >> plugins/agentic-dev-team/knowledge/foo.md
  echo "Body of second." >> plugins/agentic-dev-team/knowledge/foo.md
...
@test "bypass: --no-verify is allowed even when index is stale" {
  cd "$BATS_TMPDIR_CASE"
  echo "## New" >> plugins/agentic-dev-team/knowledge/foo.md
  echo "Body." >> plugins/agentic-dev-team/knowledge/foo.md
  git add plugins/agentic-dev-team/knowledge/foo.md
