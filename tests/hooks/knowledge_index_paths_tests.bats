...
PATHS_LIB="$BATS_TEST_DIRNAME/../../plugins/dev-team/hooks/lib/knowledge-index-paths.sh"
...
@test "_is_corpus_path matches knowledge top-level markdown" {
  run bash -c "source '$PATHS_LIB' && _is_corpus_path 'plugins/dev-team/knowledge/owasp-detection.md'"
  [ "$status" -eq 0 ]
}

@test "_is_corpus_path matches skills SKILL.md" {
  run bash -c "source '$PATHS_LIB' && _is_corpus_path 'plugins/dev-team/skills/specs/SKILL.md'"
  [ "$status" -eq 0 ]
}

@test "_is_corpus_path rejects knowledge/schemas subdirectory" {
  run bash -c "source '$PATHS_LIB' && _is_corpus_path 'plugins/dev-team/knowledge/schemas/foo.json'"
  [ "$status" -eq 1 ]
}

@test "_is_corpus_path rejects knowledge/schemas .md file" {
  # A markdown file inside knowledge/schemas/ is still out of scope.
  run bash -c "source '$PATHS_LIB' && _is_corpus_path 'plugins/dev-team/knowledge/schemas/notes.md'"
  [ "$status" -eq 1 ]
}

@test "_is_corpus_path rejects agent files" {
  run bash -c "source '$PATHS_LIB' && _is_corpus_path 'plugins/dev-team/agents/security-review.md'"
  [ "$status" -eq 1 ]
}

@test "_is_corpus_path rejects command files" {
  run bash -c "source '$PATHS_LIB' && _is_corpus_path 'plugins/dev-team/commands/code-review.md'"
  [ "$status" -eq 1 ]
}

@test "_is_corpus_path rejects docs/ files" {
  run bash -c "source '$PATHS_LIB' && _is_corpus_path 'plugins/dev-team/docs/agent-architecture.md'"
  [ "$status" -eq 1 ]
}

@test "_is_corpus_path rejects non-markdown corpus paths" {
  run bash -c "source '$PATHS_LIB' && _is_corpus_path 'plugins/dev-team/knowledge/index.json'"
  [ "$status" -eq 1 ]
}
...
@test "CORPUS_REGEX matches the same paths _is_corpus_path matches" {
  ...
  run bash -c "source '$PATHS_LIB' && echo 'plugins/dev-team/knowledge/owasp-detection.md' | grep -E \"\$CORPUS_REGEX\""
  [ "$status" -eq 0 ]
  run bash -c "source '$PATHS_LIB' && echo 'plugins/dev-team/skills/specs/SKILL.md' | grep -E \"\$CORPUS_REGEX\""
  [ "$status" -eq 0 ]
  run bash -c "source '$PATHS_LIB' && echo 'plugins/dev-team/knowledge/schemas/foo.md' | grep -E \"\$CORPUS_REGEX\""
  [ "$status" -ne 0 ]
}
