...
def test_phase_4_invokes_build():
    assert grep(r"/build", _phase_4_section())


def test_phase_4_inherits_no_refactor_mode_from_phase_0():
    assert grep(
...
def test_phase_4_measures_per_story_coverage_via_coverage_delta():
    assert grep(
...
def test_phase_4_invokes_the_mutation_kill_agent_per_story_with_file_and_max_rounds_3():
    s = _phase_4_section()
...
def test_phase_4_documents_the_c_r_w_q_mutation_kill_prompt():
...
def test_phase_4_applies_binding_mode_annotations_per_phase_0():
    assert grep(
...
def test_phase_4_review_loop_dispatches_test_design_since_and_code_review_since_in_parallel():
    s = _phase_4_section()
...
def test_phase_4_review_loop_runs_apply_fixes_corrections_then_re_runs_code_review():
    assert grep(r"/apply-fixes[[:space:]]+corrections/", _phase_4_section())


def test_phase_4_review_loop_caps_at_2_iterations_before_escalation():
    assert grep(
...
def test_phase_4_review_loop_uses_r_w_q_escalation_prompt():
    assert grep(r"\[r/w/q\]|\[r\].*\[w\].*\[q\]", _phase_4_section())


def test_phase_4_review_loop_writes_phase_4_review_json_with_fixed_schema():
    s = _phase_4_section()
    assert grep(r"memory/test-improve/<slug>/phase-4-review\.json", s)
...
def test_phase_4_review_loop_writes_waivers_json_tagged_with_findings():
    s = _phase_4_section()
