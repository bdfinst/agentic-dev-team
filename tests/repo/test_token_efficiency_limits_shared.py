...
shipped hook) and scripts/token_efficiency_review.py (the repo-root CI
runner). Both must now source the thresholds from one shared constants
module so a future policy change can't update one copy and miss the other.
"""
...
CI_SCRIPT = REPO_ROOT / "scripts" / "token_efficiency_review.py"
...
def test_ci_script_imports_shared_limits_instead_of_hardcoding() -> None:
    ...
    assert "CLAUDE_MD_CHAR_LIMIT: int = 5000" not in src, (
        "scripts/token_efficiency_review.py still hardcodes its own copy of "
