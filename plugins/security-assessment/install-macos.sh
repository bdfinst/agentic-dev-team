#!/usr/bin/env bash
# install-macos.sh — install the tools the security-assessment plugin calls.
#
...
print_help() {
  cat <<'EOF'
install-macos.sh — install security-assessment tool dependencies on macOS.
...
echo "    ./plugins/security-assessment/install.sh"
echo ""
