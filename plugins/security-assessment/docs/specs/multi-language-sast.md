...
**Target plugin:** `plugins/security-assessment`
...
This spec targets a **sibling plugin** (`plugins/security-assessment`), not the current working repo. The `/specs` skill's auto-trigger of `/plan` is intentionally suppressed — `/plan` should be invoked from within a session scoped to the target plugin, using this file as input.

To kick off implementation:
```
cd plugins/security-assessment
