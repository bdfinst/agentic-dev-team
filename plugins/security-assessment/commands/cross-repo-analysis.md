...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/harness/tools/service-comm-parser.py" <path1> <path2> ... > memory/service-comm-<slug>.mermaid
```
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/harness/tools/shared-cred-hash-match.py" <path1> <path2> ... > memory/shared-cred-<slug>.sarif
```
