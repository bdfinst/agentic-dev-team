...
| `mutation_kill_retry.py` | The retry-then-downgrade policy on repeated headless-generation failures (`is_gateway_class_error`, `make_retrying_headless_call`, `DowngradeEvent`, `GenerationExhausted`) plus its audit hook (`make_downgrade_audit_hook`) — extracted out of `mutation_kill_shared.py` once that module grew into a five-concern grab-bag; its dependencies on `mutation_kill_shared.py` are `run_claude_headless`/`resolve_fallback_model` plus `HeadlessCallFailed`, referenced only as a type (not a monkeypatchable function). |
