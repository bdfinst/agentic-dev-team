...
**Dispatch failures block the gate (issue #1752).** If step 5's `dispatchFailures` list is non-empty — any agent that failed dispatch and then failed its single retry — do not write `.review-passed`, regardless of the overall status computed from the agents that did return. The same rationale as step 3's `unreadable-registry` treatment: a lens that never ran is a coverage gap, not a passing result, so this condition is checked **before** the status check below, not folded into it. This prose rule now has a mechanical backstop (issue #1763): `hooks/pre_commit_review.py`'s `_dispatch_failure_verdict` independently vetoes the gate write when a `dispatch-failure` boundary event (emitted at Step 4, above) is on record for the current staged content — so a bug or a future caller that skips this step's condition, or writes `.review-passed` directly, still can't silently bypass it.
