...
- **Scope marker (#2166)**: append one structured, single-line marker to every dispatch prompt, listing the exact files passed under File scope above, comma-separated: `Files in scope for this review: <path>, <path>, ...`. This is metadata for the (not-yet-built) verdict recorder — it changes nothing about what gets reviewed or reported this slice.
