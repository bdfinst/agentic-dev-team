...
| --- | --- | --- | --- |
| [`/coverage-delta`](../../coverage-delta/SKILL.md) | `/test-improve` Phase 4 (per-Story coverage delta + optional mutation gate) | `/test-improve` Phase 0 ("Approach contract" — the mutation-on/off knob is the workflow-level approval point) | Phase 4 measures mutation per Story automatically when mutation is on; per-invocation prompts would block the workflow on every Story close. |
| [`/test-improve`](../../test-improve/SKILL.md) | `/test-improve` Phase 2 (baseline mutation, when mutation-on) | `/test-improve` Phase 0 (same mutation-on/off knob) | Phase 2 measures mutation across the in-scope files once before any test change so later phases have a floor; a per-invocation prompt at that boundary would break the workflow-level-approval contract. |
| [`/quality-targets-converge`](../../quality-targets-converge/SKILL.md) | `/test-improve` Phase 6 (Validate convergence loop) | `/test-improve` Phase 0 (same mutation-on/off knob) | Phase 6 runs the convergence loop under the `/test-improve` orchestrator; approval was captured once at Phase 0. `/test-improve` Phase 4 drives mutation via the `mutation-kill` agent (which wraps the tool directly and never sets this flag), so the flag enters only through this Phase-6 caller. |
