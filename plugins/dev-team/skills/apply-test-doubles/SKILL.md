...
Resolving each component's outcome writes or updates that component's Story
file for `/build` and updates the setup-guide artifact (#1436) — the same
dispatch mechanics as Step 4b's own live invocation, cited from
`../cd-test-architecture/SKILL.md`'s `### 4b. Build-vs-document decision
(off-gate adapter test doubles)` heading (Step 2's own citation, reused
here, not restated here). These are the same two artifacts Step 4b's live
invocation produces — never a third artifact, never a different mechanism.
