...
def _record(ts: str, agent: str) -> dict:
    return {
        "ts": ts,
...
def test_two_distinct_agents_in_window_both_returned(tmp_path: Path) -> None:
    ...
    result = rgc.distinct_review_agent_dispatches(tmp_path, _ANCHOR, _WINDOW)
...
def test_evaluate_missing_ledger_reason_is_missing(tmp_path: Path) -> None:
    result = rgc.evaluate(tmp_path, _ANCHOR, _WINDOW)
...
def _doc_only_exempt(ts: str) -> dict:
    return {
        "ts": ts,
        "hook": "code-review",
        "tool": "Skill",
...
def test_doc_only_exemption_inside_window_is_true(tmp_path: Path) -> None:
    ...
    assert rgc.has_doc_only_exemption(tmp_path, _ANCHOR, _WINDOW) is True
...
def test_doc_only_exemption_outside_window_is_false(tmp_path: Path) -> None:
    ...
    assert rgc.has_doc_only_exemption(tmp_path, _ANCHOR, _WINDOW) is False
