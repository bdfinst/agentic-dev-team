"""Parity fixtures for hooks/cost-meter (#597).

Runs the .sh and .py implementations against six fixture directories under
fixtures/cost_meter/. assert_parity fails the merge on any byte-level
divergence in stdout, exit code, normalized stderr, or side-effect tree.

The happy-path fixture invokes the shared lib/cost_meter.py `record`
subcommand — both sides shell out to the same Python module so any timestamp
in the log entry appears identically. stderr normalization scrubs ISO-8601
stamps regardless.
"""
...
_HOOK_SH = _REPO_ROOT / "plugins" / "dev-team" / "hooks" / "cost-meter.sh"
_HOOK_PY = _REPO_ROOT / "plugins" / "dev-team" / "hooks" / "cost_meter.py"
_FIXTURES_DIR = Path(__file__).resolve().parent / "fixtures" / "cost_meter"
