"""Parity fixtures for scripts/issue-deps (#614 / #572 Phase 3).

Fixture layout (per subdirectory of fixtures/issue_deps/):
    argv.json          - JSON list of argv tokens (defaults to ["plan.md"])
    initial_tree/      - files copied into sandbox (plan.md is placed here)
"""
...
_HOOK_SH = _REPO_ROOT / "plugins" / "dev-team" / "scripts" / "issue-deps.sh"
_HOOK_PY = _REPO_ROOT / "plugins" / "dev-team" / "scripts" / "issue_deps.py"
_FIXTURES_DIR = Path(__file__).resolve().parent / "fixtures" / "issue_deps"
...
def test_parity(fixture_dir: Path) -> None:
    if shutil.which("jq") is None:
        pytest.skip("jq required by issue-deps.sh")
