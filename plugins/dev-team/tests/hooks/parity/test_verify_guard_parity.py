"""Parity fixtures for hooks/verify-guard (#608 / #572 Phase 3).

Fixture layout (per subdirectory of fixtures/verify_guard/):
    stdin.json              - PreToolUse payload
    env.json                - {name: value} overlay (TMPDIR points into sandbox)
    initial_tree/           - pre-seeded state files
                              (e.g. tmp/dev-team-verify-guard/<sid>.json)

Both implementations use `${TMPDIR:-/tmp}` for the state dir, so pointing
TMPDIR into the sandbox keeps state fully hermetic.
"""
...
_HOOK_SH = _REPO_ROOT / "plugins" / "dev-team" / "hooks" / "verify-guard.sh"
_HOOK_PY = _REPO_ROOT / "plugins" / "dev-team" / "hooks" / "verify_guard.py"
_FIXTURES_DIR = Path(__file__).resolve().parent / "fixtures" / "verify_guard"
...
def _load_fixture(dirpath: Path) -> Fixture:
    ...
    # Route state through the sandbox by default so parity is hermetic.
