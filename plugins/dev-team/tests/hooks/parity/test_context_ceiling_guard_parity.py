"""Parity fixtures for hooks/context-ceiling-guard (#595).

Runs the .sh and .py implementations against six fixture directories under
fixtures/context_ceiling_guard/. assert_parity fails the merge on any
byte-level divergence in stdout, exit code, normalized stderr, or side-effect
tree (which for this hook is the per-session dedupe marker under $TMPDIR).

Fixture layout (per subdirectory of fixtures/context_ceiling_guard/):
    stdin.json              - stdin payload for the hook (required)
    env.json                - {name: value} env overlay (optional; supports
                              the literal string "SANDBOX" as a value, which
                              is replaced with the sandbox tmpdir per side)
    initial_tree/           - files copied into the sandbox root before run
                              (typically transcript.jsonl the hook reads)
"""
...
_HOOK_SH = _REPO_ROOT / "plugins" / "dev-team" / "hooks" / "context-ceiling-guard.sh"
_HOOK_PY = _REPO_ROOT / "plugins" / "dev-team" / "hooks" / "context_ceiling_guard.py"
_FIXTURES_DIR = Path(__file__).resolve().parent / "fixtures" / "context_ceiling_guard"
