...
**Notes.** Override services in `WebApplicationFactory.WithWebHostBuilder(...ConfigureTestServices)` to double outbound dependencies pre-merge without config. Inject `TimeProvider` (or an `IClock`) for time determinism; never `DateTime.Now` in logic — pair with Polly v8 bound to the same `TimeProvider` so retry/backoff/circuit-breaker policies advance virtual time deterministically. For the **API Consumer** side (your service calling out), the canonical pre-merge seam is `HttpMessageHandler` — not `WebApplicationFactory`, not Kestrel on a port — wrapped behind a thin owned adapter; WireMock.Net is the preferred implementation of that seam when installed (still `HttpMessageHandler`-shaped — the library backs the stub, it doesn't replace the seam), with the hand-rolled `StubHttpMessageHandler` kept as backup; see `../references/csharp-http-client-testing.md` for the full pattern (`StubHttpMessageHandler`, request-shape assertions, retry sequencing, off-the-shelf alternatives, smells, and the WireMock.Net preferred/backup detail). Avoid the in-memory EF provider for anything relational — it hides SQL bugs; use Testcontainers or SQLite-in-memory deliberately and know its limits.
