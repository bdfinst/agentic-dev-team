...
| `/gherkin-derive` | `skills/gherkin-derive/SKILL.md` | worker | Standalone Gherkin derivation from code — discovers the public surface (OpenAPI → routes → tests → signatures), recommends a BDD binding mode via the value rubric, writes `.feature` files and (bdd-runner) pending step stubs; no `/test-modernize` map or tracker Stories. Phase 1b of `/test-upgrade` |
...
| `/test-improve` | `skills/test-improve/SKILL.md` | orchestrator | Consolidated analyze-then-improve test orchestrator (replaces `/test-modernize` and `/test-upgrade`) — 7 phases: Phase-0 approach contract → `/test-health` analyze → coverage + mutation baseline → optional `/gherkin-derive` → `/issues-from-assessment` triage → `/build` + `/coverage-delta --workflow test-improve` + `mutation-kill` per Story (no-refactor) → optional refactor-for-testability → `/quality-targets-converge` → 10-section executive-summary report. Defaults to lightweight ceremony; opts into heavier capabilities only when the operator asks; Go-aware (mutation advisory); local-first with `--parent <url>` opt-in for tracker sinks |
...
| `/test-upgrade` | `skills/test-upgrade/SKILL.md` | orchestrator | General-purpose analyze-then-improve test workflow for any JS/Go/Java/C# codebase — 4 phases: `/test-health` analyze → optional `/gherkin-derive` → `/issues-from-assessment` triage → `/build` + `/coverage-delta --workflow test-upgrade` + `mutation-kill` per Story → `/quality-targets-converge`. Local-first, BDD optional, Go-aware (mutation advisory); no-refactor default with refactor-allowed opt-in |
