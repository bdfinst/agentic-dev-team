...
| Consolidated analyze-then-improve orchestrator — lightweight by default, opts into heavier capabilities (Gherkin, mutation, refactor-for-testability) only when asked; always baselines before changing tests; ships a 10-section executive-summary report | `/test-improve` | Whole repository | Remediation (consolidated) |
| Modernize a legacy repository's tests to hit CD targets (≥ 90% coverage, zero surviving mutants, 100% deterministic, fastest pre-merge wall-clock) | `/test-modernize` | Whole repository | Remediation (legacy CD focus) |
...
**`/test-improve`** is the **consolidated remediation altitude** and the default choice for new work — replaces `/test-modernize` and `/test-upgrade` with a single seven-phase orchestrator (approach contract → analyze → baseline → optional Gherkin → triage → improve-without-refactoring → optional refactor-for-testability → validate → executive-summary report). It defaults to lightweight ceremony (mutation off, BDD `none`, no-refactor) and prompts for heavier capabilities on demand. Phase 1 delegates the entire analysis to `/test-health`. See the workflow diagram in [Architecture](agent-architecture.md#test-improvement-workflow-test-improve).

**`/test-modernize`** is the *legacy-CD-focused remediation* altitude — what to do with a `cd-test-architecture` assessment. It sequences five gated phases (analysis → public-interface Gherkin → audit + baseline coverage → no-refactor tests added → minimum refactor and converge on quality targets), holds the human gate between each phase, and writes Phase-tagged Stories to the tracker the parent issue URL points at (GitHub, ADO, GitLab, Jira — or local plan files when no URL or CLI is available). It does not invent its own assessment; Phase 1 invokes `/cd-test-architecture` directly. See the workflow diagram in [Architecture](agent-architecture.md#test-modernization-workflow).
