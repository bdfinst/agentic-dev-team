...
**`/test-improve`** is the **consolidated remediation altitude** — a seven-phase orchestrator (approach contract → analyze → baseline → optional Gherkin → plan fixes → improve-without-refactoring → optional refactor-for-testability → validate → executive-summary report). It defaults to lightweight ceremony (mutation off, BDD `none`, no-refactor) and prompts for heavier capabilities on demand. Phase 1 delegates the entire analysis to `/test-health`, which in turn folds in `cd-test-architecture` (advisory), `/test-design`, and `mutation-testing`. See the workflow diagram in [Architecture](agent-architecture.md#test-improvement-workflow-test-improve).
