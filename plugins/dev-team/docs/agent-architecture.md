...
5. **Refactor-for-testability + converge** — for each Phase-5 Story (predecessor: its `[Baseline]` Story must be green first), `/build` lands the minimum behavior-preserving refactor plus the test that needed the new seam. Phase-5 `[Component tests]` Stories also bind to approved Scenarios. `/quality-targets-converge` runs after every Story; it **reuses Phase-4's `mutation-history.json`** for files whose last commit pre-dates the recorded entry — only stale or absent files are measured fresh — then the loop picks the largest gap to the four targets and dispatches the smallest action to close it. When the action is "add a component test for a new behavior" and no approved Scenario covers it, the loop pauses and hands back to the orchestrator so the operator can update the Gherkin surface via the standard Phase-2 sign-off — the operator remains the only author of intent, and the loop does not open ad-hoc amendment Stories from inside a worker. Same end-of-phase review loop fires (`/test-design` + `/code-review` + `/apply-fixes`, max 2 iterations) writing `phase-5-review.json` before the human gate. Targets are met or each remaining gap is explicitly waived with a recorded reason.
