...
1. **Analyze.** `/test-health` is the sole worker; it internally rolls up whatever sub-skills it needs. The Phase-0 mutation setting flows through. A separate, direct classification pass (independent of `/test-health`) persists a before-snapshot of test counts by MinimumCD type to `test-counts-before.json`.
