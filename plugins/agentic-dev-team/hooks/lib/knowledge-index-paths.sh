...
#
# The corpus is:
#   - plugins/agentic-dev-team/knowledge/*.md         (top-level .md only)
#   - plugins/agentic-dev-team/skills/<name>/SKILL.md
#
# Excluded:
#   - plugins/agentic-dev-team/knowledge/schemas/**   (json schemas, not docs)
...
# ERE form for grep -E.
CORPUS_REGEX='(^|/)plugins/agentic-dev-team/(knowledge/[^/]+\.md|skills/[^/]+/SKILL\.md)$'

# _is_corpus_path <path>
# Returns 0 (true) when the path matches the indexed corpus, 1 otherwise.
_is_corpus_path() {
  [[ "$1" =~ /?plugins/agentic-dev-team/(knowledge/[^/]+\.md|skills/[^/]+/SKILL\.md)$ ]]
}
