...
- [ ] AC9: **All four** `evals/expected/*.test.json` files naming `test-smell-review` under `applicableAgents` (`test-assertion-roulette`, `test-mystery-guest`, `test-clean-doubles`, `test-overspecified-mocks`) require the appropriate `remedyFamily` cite via `mustMention` on the family slug — matched by `verdict.py` against `issue.message` + `summary` prose. Mapping: Assertion Roulette → `result-verification`; Mystery Guest → `fixture-construction`; Overspecified Mocks → `test-organization` (Custom Assertion / behavior-vs-state); Clean Doubles → `null` allowed (no smell → no family), so no addition beyond leaving `mustMention` unchanged. Confirm each mapping against `test-smells.md` during Step 2.1.
...
- **Overspecified Mocks family mapping.** The plan maps Overspecified Software (over-mocking) → `test-organization` (verify state, not internal calls). If Step 2.1's `test-smells.md` confirmation lands on a different best family (e.g. `test-refactoring` for a _Replace Mock with Stub_ move, or a `test-doubles`-family cite outside the four allowed values), record the deviation in the Ambiguity Log and either widen the allowed-values set to include it or restate the mapping. Do NOT silently pick a family that lacks knowledge-file support.
