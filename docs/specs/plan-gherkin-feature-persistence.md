...
**A11 — Overwrites and writes are never silent.** The export reports the destination and count of files written; a re-export over existing derived files reports the overwrite count. Asserted by `plan_gherkin_export.py` unit tests.
