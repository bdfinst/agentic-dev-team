...
| AC15 | Performance | Bats: 1000 sequential `model-resolve.sh haiku` invocations (happy path, no override) complete in < 50s wall-clock on a baseline laptop (50ms p99 ceiling per invocation). Apple Silicon measurements: ~10-15ms per invocation, dominated by bash + jq cold-start. |
