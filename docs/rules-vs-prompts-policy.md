...
**Date**: 2026-04-24
**Applies to**: `plugins/dev-team/agents/security-review.md` (agent prompt) and `plugins/security-assessment/knowledge/semgrep-rules/*.yaml` (custom rulesets) plus their interaction with community rulesets (`p/security-audit`, `p/owasp-top-ten`).
...
Each custom rule in `plugins/security-assessment/knowledge/semgrep-rules/*.yaml` must ship with:
...
The agent emits findings with `rule_id` fields in the unified finding envelope via the adapter at `plugins/dev-team/skills/static-analysis-integration/adapters/security-review-adapter.py`. Pattern-visible classes adopt upstream rule_ids (e.g. `semgrep.generic.sql-injection`); judgment-only classes use the `security-review.a<NN>.<slug>` namespace. The canonical mapping lives at `plugins/dev-team/knowledge/security-review-rule-map.yaml`.

Cross-source dedup collapse (agent + semgrep on the same issue) works because the adapter adopts upstream rule_ids, matching fp-reduction's Stage 4 collapse key `rule_id + file + line` (`plugins/security-assessment/agents/fp-reduction.md:57`).

## Out of scope

- Red-team probe interpretation (lives in `plugins/security-assessment/agents/redteam-*.md`). Those agents are narrative synthesizers over tool output, not detectors.
