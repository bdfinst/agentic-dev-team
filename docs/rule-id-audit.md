...
1. **[RESOLVED by Item 5]** **Blocker: agent findings have no `rule_id`.** The agent's output schema originally emitted `{status, issues: [{severity, confidence, file, line, message, suggestedFix}], summary}` with no `rule_id`. Item 5 (`plugins/dev-team/docs/specs/agent-rule-id-adapter.md`) added a required `category` field to agent output, built the adapter at `plugins/dev-team/skills/static-analysis-integration/adapters/security-review-adapter.py`, and published the canonical mapping at `plugins/dev-team/knowledge/security-review-rule-map.yaml`. Adapter emits the unified-finding envelope with correct `rule_id`. Fp-reduction Stage 4 collapse key was also clarified to `rule_id + file + line` (see `plugins/security-assessment/agents/fp-reduction.md:57`), which enables cross-source collapse when the adapter adopts upstream rule_ids per policy Q4.
