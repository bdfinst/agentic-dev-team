# Rule ID dedup audit — `security-review` agent vs. `security-assessment` plugin rulesets

**Date**: 2026-04-24
**Scope**: All detection output that lands in the unified-finding envelope defined by `plugins/dev-team/knowledge/security-primitives-contract.md`
...
Three findings. **All three are now resolved** — see status annotations below.

1. **[RESOLVED by Item 5]** **Blocker: agent findings have no `rule_id`.** The agent's output schema originally emitted `{status, issues: [{severity, confidence, file, line, message, suggestedFix}], summary}` with no `rule_id`. Item 5 (`plugins/dev-team/skills/static-analysis-integration/references/security-review-adapter.md`) added a required `category` field to agent output, built the adapter at `plugins/dev-team/skills/static-analysis-integration/adapters/security-review-adapter.py`, and published the canonical mapping at `plugins/dev-team/knowledge/security-review-rule-map.yaml`. Adapter emits the unified-finding envelope with correct `rule_id`. Fp-reduction Stage 4 collapse key was also clarified to `rule_id + file + line` (see `plugins/security-assessment/agents/fp-reduction.md:57`), which enables cross-source collapse when the adapter adopts upstream rule_ids per policy Q4.
2. **[RESOLVED by Item 5 + Item 3b]** **Dedup gap: pattern-visible classes.** Item 5's mapping table adopts upstream semgrep rule_ids for the 11 pattern-visible classes named in `docs/rules-vs-prompts-policy.md`. Item 3b then stripped those classes from `owasp-detection.md` (replaced with pointer stubs) so the agent no longer independently detects them — it only assesses exploitability on tool findings. Combined, the agent and semgrep emit identical rule_ids on the same issue and collapse cleanly.
3. **[PARTIALLY RESOLVED — coverage gap remains]** **Coverage gap: agent covers classes with no plugin rule.** Judgment-tier classes (IDOR, missing auth middleware, no rate limiting, session fixation, PII in logs, etc.) remain agent-only by design per the rules-vs-prompts policy. This is intended, not a gap — these classes require cross-file or architectural reasoning that a rule cannot encode reliably. The ~8 originally-flagged classes are now in the agent's `A<NN>.<slug>` namespace and land in the unified finding stream via the adapter's `security-review.a<NN>.<slug>` rule_id fallback.
